...
public class CustomInterfaces extends RSInterface {
	 ...
	 * @param Valius
	 */
	public static void unpackInterfaces(TextDrawingArea[] Mayhem) {
		...
		/*
		 * Handles teleporting to various locations around Valius
		 */
		trainingTeleport(Mayhem);
		skillingTeleport(Mayhem);
		pvpTeleport(Mayhem);
		bossTeleport(Mayhem);
		minigameTeleport(Mayhem);
		otherTeleport(Mayhem);
		 ...
		 */
		settings(Mayhem);
		screenOptions(Mayhem);

		/*
		 * Handles the achievement tab (Lists all the achievements)
		 */
		achievementInterface(Mayhem);
		//achievementsTab(Mayhem);
		...
		/*
		 * Handles the bucks system for Valius
		 */
		creditsTab(Mayhem);
		 ...
		 */
		pestControlBoat(Mayhem);
		pestControlGame(Mayhem);
		
		/*
		 * Handles Player Profiler interface
		 */
		playerProfiler(Mayhem);
		myProfile(Mayhem);
		profileTab(Mayhem);
		profileLeaderboards(Mayhem);
		 ...
		 */
		bankPinManager(Mayhem);
		
		/**
		 * Account Manager
		 */
		accountManager(Mayhem);
		 ...
		 */
		houseSidebar(Mayhem);
		houseMainScreen(Mayhem);
	}
	
	/**
	 * Interface for changing game play options
	 * @param tda
	 */
	public static void settings(TextDrawingArea[] tda) {
		...
		addText(28505, "Settings Manager", tda, 2, 0xFF981F, true, true);
		addText(28506, "Customize Valius to your likings.", tda, 1, 0xFF981F, true, true);
		addHoverButton(28507, 513, 37, 25, "Default settings", -1, 28508, 1);
		addHoveredButton(28508, 514, 37, 25, 28509);
		addText(28510, "Reset", tda, 0, 0xFF981F, false, true);
		rsi.totalChildren(9);
		rsi.child(0, 28501, 11, 6);
		rsi.child(1, 28502, 470, 15);
		rsi.child(2, 28503, 470, 15);
		rsi.child(3, 28505, 255, 15);
		rsi.child(4, 28506, 255, 57);
		rsi.child(5, 28550, 30, 109);
		rsi.child(6, 28507, 422, 53);
		rsi.child(7, 28508, 422, 53);
		rsi.child(8, 28510, 425, 59);
		RSInterface scrollInterface = addTabInterface(28550);
		scrollInterface.scrollPosition = 0;
		scrollInterface.contentType = 0;
		scrollInterface.width = 431;
		scrollInterface.height = 202;
		scrollInterface.scrollMax = 300;
		scrollInterface.totalChildren(84);
		int child = 0;
		for (int i = 0; i < 21 * 5; i += 5) {
			int x = (i/5) % 3;
			int y = i / 5 / 3;
			x *= 113 + 30;
			y *= 34 + 5;
			addHoverButton(28551 + i, 515, 113, 34, "Toggle", -1, 28551 + (i + 1), 1);
			addHoveredButton(28551 + (i + 1), 516, 113, 34, 28551 + (i + 2));
			addText(28551 + i + 3, "", tda, 0, 0xFF981F, true, true);
			addText(28551 + i + 4, "", tda, 3, 0x007BFF, true, true);
			scrollInterface.child(child++, 28551 + i, x + 20, y + 20);
			scrollInterface.child(child++, 28551 + (i + 1), x + 20, y + 20);
			scrollInterface.child(child++, 28551 + (i + 3), x + 20 + 113 / 2, y + 23);
			scrollInterface.child(child++, 28551 + (i + 4), x + 20 + 113 / 2, y + 33);
		}
	}
	 ...
	 * @param tda
	 */
	public static void playerTitles(TextDrawingArea[] tda) {
		//INTERFACE_COUNT++;
		RSInterface rsi = addInterface(39000);
		addSprite(39001, 266);
		addText(39002, "Player Title Manager", tda, 2, 0xFF981F, true, true);
		addText(39003, "<u=255>Drop-out", tda, 3, 0xFF981F, true, true);
		...
		addText(39022, "Color", tda, 0, 0xFF981F, false, true);
		rsi.totalChildren(19);
		rsi.child(0, 39001, 55, 42);
		rsi.child(1, 39002, 325, 60);
		rsi.child(2, 39003, 325, 100);
		rsi.child(3, 39050, 58, 48);
		rsi.child(4, 39004, 270, 130);
		rsi.child(5, 39005, 233, 155);
		rsi.child(6, 39006, 233, 172);
		rsi.child(7, 39007, 233, 188);
		...
		rsi.child(16, 39019, 375, 235);
		rsi.child(17, 39020, 375, 235);
		rsi.child(18, 39022, 378, 241);
		...
		for (int i = 0; i < 10 * 8; i += 4) {
			addHoverButton(39051 + i, 277, 278, 114, "Select", -1, 39051 + (i + 1), 1);
			addHoveredButton(39051 + (i + 1), 278, 114, 26, 39051 + (i + 2));
			addText(39051 + (i + 3), "ID: " + (39051 + (i + 3)), tda, 0, 0xFF981F, false, true);
			scrollInterface.child(child++, 39051 + i, x, y);
			scrollInterface.child(child++, 39051 + (i + 1), x, y);
			scrollInterface.child(child++, 39051 + (i + 3), x + 5, y + 9);
			y += 25;
		}
	}  
	
	/**
	 * Quest tab interface which displays main information about Mayhem.
	 * 
	 * @param tda
	 */
	public static void questTab(TextDrawingArea tda[]) {
		...
		addText(29406, "localhost", tda, 0, 0xF7AA25, true, true);
		addText(29407, "Information", tda, 2, 0xF7AA25, true, true);
		rsinterface.totalChildren(8);
		rsinterface.child(0, 29401, 2, 38);
		rsinterface.child(1, 29402, 2, 0);
		rsinterface.child(2, 29403, 49, 0);
		rsinterface.child(3, 29404, 96, 0);
		rsinterface.child(4, 29405, 143, 0);
		rsinterface.child(5, 29406, 95, 238);
		rsinterface.child(6, 29407, 95, 48);
		rsinterface.child(7, 29450, 9, 74);
		RSInterface scrollInterface = addTabInterface(29450);
		scrollInterface.scrollPosition = 0;
		scrollInterface.contentType = 0;
		scrollInterface.width = 155;
		scrollInterface.height = 156;
	...
	public static void achievementInterface(TextDrawingArea[] TDA) {
		...
		addText(35005, "Achievement Handler", TDA, 2, 0xF7AA25, true, true);
	...
	private static void houseSidebar(TextDrawingArea[] Mayhem) {
		RSInterface sidebar = addInterface(53000);
		addText(53001, "House Settings", Mayhem, 2, 0xff9933, true, true);
		...
		addText(53004, "Build Mode:", Mayhem, 2, 0xffffff, false, true);
		addText(53005, "House Lock:", Mayhem, 2, 0xffffff, false, true);
		...
		addText(53027, "Leave House", Mayhem, 2, 0xffffff, false, true);
		addText(53028, "Grab Tools", Mayhem, 2, 0xffffff, false, true);
		addText(53029, "Build Object", Mayhem, 2, 0xffffff, false, true);
		addText(53030, "Ban Guest", Mayhem, 2, 0xffffff, false, true);
		addText(53031, "Unban Guest", Mayhem, 2, 0xffffff, false, true);
		addText(53032, "Expel Guest", Mayhem, 2, 0xffffff, false, true);
		addText(53033, "Expel Guests", Mayhem, 2, 0xffffff, false, true);

		sidebar.totalChildren(26);
		sidebar.child(0, 53001, 100, 4);
		sidebar.child(1, 53002, 15, 25);
		sidebar.child(2, 53003, 15, 50);
		sidebar.child(3, 53004, 19, 28);
		sidebar.child(4, 53005, 19, 53);
		sidebar.child(5, 53006, 55, 115);
		sidebar.child(6, 53007, 55, 115);
		sidebar.child(7, 53009, 5, 150);
		sidebar.child(8, 53010, 5, 150);
		sidebar.child(9, 53012, 100, 150);
		sidebar.child(10, 53013, 100, 150);
		sidebar.child(11, 53015, 5, 185);
		sidebar.child(12, 53016, 5, 185);
		sidebar.child(13, 53018, 100, 185);
		sidebar.child(14, 53019, 100, 185);
		sidebar.child(15, 53021, 5, 220);
		sidebar.child(16, 53022, 5, 220);
		sidebar.child(17, 53024, 100, 220);
		sidebar.child(18, 53025, 100, 220);
		sidebar.child(19, 53027, 58, 121);
		sidebar.child(20, 53028, 13, 156);
		sidebar.child(21, 53029, 103, 156);
		sidebar.child(22, 53030, 14, 191);
		sidebar.child(23, 53031, 103, 191);
		sidebar.child(24, 53032, 10, 226);
		sidebar.child(25, 53033, 102, 226);
	}
	
	public static void houseMainScreen(TextDrawingArea[] Mayhem) {
		RSInterface main = addInterface(53100);
		RSInterface list = addInterface(53123);
		int lines = 100;
		list.totalChildren(lines);
		list.childY[0] = 2;
		// System.out.println(id);
		for (int index = 53124; index < 53124 + lines; index++) {
			addHoverText(index, "", "Select", Mayhem, 1, 0xffffff, false, true, 120);
			list.children[index - 53124] = index;
			list.childX[index - 53124] = 5;
			list.childY[index - 53124] = (index - 53124 > 0 ? list.childY[index - 53124 - 1] + 14 : 2);
		}
		list.width = 139;
		list.height = 210;
		...
		addText(53110, "Build", Mayhem, 2, 0xffffff, false, true);
		itemContainer(53111, 5, 7, 8, 3, false);
		addText(53112, "Materials:", Mayhem, 2, 0xffbb55, false, true);
		addText(53113, "Rotation:", Mayhem, 2, 0xffbb55, false, true);
		addText(53114, "Build an Object", Mayhem, 2, 0xff9933, false, true);
		addText(53115, "Select an Object", Mayhem, 2, 0xffffff, true, true);
		addText(53116, "0", Mayhem, 2, 0xffffff, false, true);
		addHoverText(53117, "All", "Show All Objects", Mayhem, 1, 0xff0000, true, true, 65);
		addHoverText(53118, "Ores", "Show Only Ores", Mayhem, 1, 0xff0000, true, true, 65);
		addHoverText(53119, "Trees", "Show Only Trees", Mayhem, 1, 0xff0000, true, true, 65);
		addHoverText(53120, "Altars", "Show Only Altars", Mayhem, 1, 0xff0000, true, true, 65);
		addHoverText(53121, "Stalls", "Show Only Stalls", Mayhem, 1, 0xff0000, true, true, 65);
		addHoverText(53122, "Misc.", "Show Only Misc.", Mayhem, 1, 0xff0000, true, true, 65);

		main.totalChildren(21);
		main.child(0, 53101, 0, 0);
		main.child(1, 53102, 487, 8);
		main.child(2, 53103, 487, 8);
		main.child(3, 53105, 236, 270);
		main.child(4, 53106, 268, 270);
		main.child(5, 53107, 438, 293);
		main.child(6, 53108, 438, 293);
		main.child(7, 53110, 453, 301);
		main.child(8, 53111, 192, 136);
		main.child(9, 53112, 173, 109);
		main.child(10, 53113, 173, 269);
		main.child(11, 53114, 203, 8);
		main.child(12, 53115, 333, 40);
		main.child(13, 53116, 256, 271);
		main.child(14, 53117, 15, 39);
		main.child(15, 53118, 89, 39);
		main.child(16, 53119, 15, 63);
		main.child(17, 53120, 89, 63);
		main.child(18, 53121, 15, 87);
		main.child(19, 53122, 89, 87);
		main.child(20, 53123, 7, 117);
	}
	
	public static void screenOptions(TextDrawingArea[] Mayhem) {
		RSInterface tab = addInterface(28200);
		addSprite(28201, 472);
		addText(28202, "Screen Options", Mayhem, 2, 0xff9933, true, true);
		...
		addText(28209, "Transparent side panel", Mayhem, 1, 0xff9933, false, true);
		addText(28210, "Make each side panel transparent", Mayhem, 0, 0xff9933, false, true);		
		addText(28211, "Transparent chatbox", Mayhem, 1, 0xff9933, false, true);
		addText(28212, "Make chatbox transparent", Mayhem, 0, 0xff9933, false, true);	
		addText(28213, "Side-stones arrangement", Mayhem, 1, 0xff9933, false, true);
		addText(28214, "Change the side-stones arrangement", Mayhem, 0, 0xff9933, false, true);		
		addText(28215, "Settings are not applicable in fixed mode!", Mayhem, 0, 0xff9933, true, true);
	...
	public static void accountManager(TextDrawingArea[] daniel) {
		...
		addText(56007, "Having a safe account is crucial in the world of Mayhem.", daniel, 0, 0xff9933, true, true);
	...
	public static void reportAbuse(TextDrawingArea[] daniel) {
		...
		String[] rules = { 
			...
			"Valius staff impersonation", "Account sharing/trading", "Macroing", "Multiple logging in",
	...
	public static String[][][] shopContent = new String[][][] {
		...
		/** Colors */
		{ 
			{ "<col=C22323>Red", "@whi@50" }, 
			{ "<col=0FA80F>Green", "@whi@50" }, 
			{ "<col=2AA4C9>Blue", "@whi@50" },
			{ "<col=C9BC28>Yellow", "@whi@50" },
			{ "<col=F58D16>Orange", "@whi@50" },
			{ "<col=C931E8>Purple", "@whi@100" },
			{ "<col=F52CD7>Pink", "@whi@50" },
			{ "<col=FFFFFF>White", "@whi@50" }, 
		} 
	};
	
	public static void fireColor(TextDrawingArea[] Mayhem) {
		RSInterface tab = addInterface(49750);
		addSprite(49751, 400);
		addHoverButton(49752, 21, 21, 21, "Close", 250, 49753, 3);
		addHoveredButton(49753, 22, 21, 21, 49754);
		addText(49755, "Fire Color Changer", 0xff9933, true, true, -1, Mayhem, 2);
		addText(49756, "By paying 10 Valius Bucks you can change all fire colors.", 0xff9933, true, true, -1, Mayhem, 0);
		addText(49757, "You will earn double cooking & firemaking exp for 30mins.", 0xff9933, true, true, -1, Mayhem, 0);
		addText(49758, "Current Color: Orange", 0xff9933, true, true, -1, Mayhem, 0);
		addText(49759, "Changed by: Daniel", 0xff9933, true, true, -1, Mayhem, 0);
		addText(49760, "Daniel's Clan: Valius", 0xff9933, true, true, -1, Mayhem, 0);
		addText(49761, "Change availability: 45minutes", 0xff9933, true, true, -1, Mayhem, 0);
	...
	public static void starter(TextDrawingArea[] daniel) {
		...
		addSprite(51751, 431);
		addText(51752, "Valius Account Setup", 0xff9933, true, true, 52, daniel, 2);
	...
	public static void loyaltyShop(TextDrawingArea[] tda) {
		...
		addSprite(id + 6, 407);
		addText(id + 7, "Valius Bucks", tda, 0, 0xe6be78, false, true);
		addText(id + 8, "Welcome to the Valius title shop.", tda, 0, 0xe6be78, false, true);
	...
	public static void tracker(TextDrawingArea[] Mayhem) {
		RSInterface tab = addInterface(54100);
		addSprite(54101, 371);
		addHoverButton(54102, 17, 21, 21, "Close", 250, 54103, 3);
		addHoveredButton(54103, 18, 21, 21, 54104);
		addText(54105, "Tracker", 0xff9933, true, true, -1, Mayhem, 2);
		addText(54106, "Show's a list of tracked statistics.", 0xff9933, true, true, -1, Mayhem, 0);
		tab.totalChildren(6);
		...
		int y = 0;
		for (int i = 0; i < 50; i++) {
			addHoverText(54111 + i, "", "", Mayhem, 0, 0xcc8000, true, true, 300);
	 ...
	 * @param Mayhem
	 */
	public static void bankSettings(TextDrawingArea[] Mayhem) {
		RSInterface tab = addInterface(32500);
		addSprite(32501, 368);
		addText(32502, "Valius Bank Settings", 0xff9933, true, true, -1, Mayhem, 2);
		...
		addText(32509, "First item in tab", 0xff9933, true, true, -1, Mayhem, 1);
		addText(32510, "Digit (1, 2, 3)", 0xff9933, true, true, -1, Mayhem, 1);
		addText(32511, "Roman numeral (I, II, III)", 0xff9933, true, true, -1, Mayhem, 1);
		addHoverText(32512, "Back to bank", "View", Mayhem, 1, 0xcc8000, true, true, 100, 0xFFFFFF);
		tab.totalChildren(11);
	...
	*/
	...
	public static void friendsTab(TextDrawingArea[] tda) {
		...
		addText(5070, "   Valius", tda, 0, 0xff9933, false, true);
	...
	public static void ignoreTab(TextDrawingArea[] tda) {
		...
		addText(5720, "   Valius", tda, 0, 0xff9933, false, true);
	...
	public static void itemDetails(TextDrawingArea[] Mayhem) {
		RSInterface tab = addInterface(59750);
		addSprite(59751, 456);
		addText(59752, "Item Details:", 0xff9933, true, true, -1, Mayhem, 2);
		addText(59753, "</col>Item: @gre@Armadyl Godsword", 0xff9933, true, true, -1, Mayhem, 0);
		addText(59754, "</col>General Price: @gre@85M", 0xff9933, true, true, -1, Mayhem, 0);
		addText(59755, "</col>Tradeable: @gre@True", 0xff9933, true, true, -1, Mayhem, 0);
		addText(59756, "</col>Noted: @gre@False", 0xff9933, true, true, -1, Mayhem, 0);
		itemDisplay(59757, 30, 10, 5, 5, "Complain");
		addHoverText(59758, "-> Back <-" , "Go back to drop table", Mayhem, 0, 0xff9933, true, true, 300);
		tab.totalChildren(8);
	 ...
	 * @param Mayhem
	 */
	public static void dropTable(TextDrawingArea[] Mayhem) {
		RSInterface tab = addInterface(59800);
		addSprite(59801, 16);
		addHoverButton(59802, 454, 17, 17, "Close", 250, 59803, 3);
		addHoveredButton(59803, 455, 17, 17, 59804);
		addText(59805, "Monster Drop Guide", 0xff9933, true, true, -1, Mayhem, 2);	
		addText(59806, "Name:", 0xff9933, true, false, -1, Mayhem, 0);
		addText(59807, "Level:", 0xff9933, true, false, -1, Mayhem, 0);
		addText(59818, "", 0xff9933, true, false, -1, Mayhem, 0);	
		String[] table = { "Always", "Common", "Uncommon", "Rare", "Very Rare" };	
		for (int i = 0; i < table.length; i++) {
			addText(59808 + i, table[i], 0x000000, true, false, -1, Mayhem, 0);			
		}	
		...
		int y = 0;
		for (int i = 0; i < 50; i++) {
			addHoverText(59901 + i, "" , "", Mayhem, 0, 0xCF4F0A, false, true, 300);
	...
	public static void mysteryBox(TextDrawingArea[] daniel) {
		...
		addHoverButton(17004, 258, 146, 37, "Bet Valius Bucks", -1, 17005, 1);
		addHoveredButton(17005, 259, 146, 37, 9454);
		addText(17006, "</col>Valius Bucks: @gre@1,000", daniel, 0, 0xF7AA25, true, true);
		...
		addText(17008, "Mystery Box is a gambling minigame where you can bet 1 Valius Buck.", daniel, 0, 0xF7AA25, true, true);
		...
		addHoverText(17011, "Visit online store", "@red@Donate to Valius!", daniel, 0, 0xF7AA25, false, true, 168);
	...
	public static void itemsOnDeath(TextDrawingArea[] daniel) {
		...
		addText(17105, "Items you lose on death if not skulled:", daniel, 0, 0xff981f);
	...
	public static void creditsTab(TextDrawingArea[] daniel) {
		...
		addSprite(52501, 38);
		addText(52502, "Valius Shops", 0xff9933, true, true, -1, daniel, 2);
		addSprite(52503, 55);
		addText(52504, "Current Valius Bucks:", 0xff9933, true, true, -1, daniel, 0);
		addHoverText(52505, "Visit online store", "@red@Donate to Valius!", daniel, 0, 0xF7AA25, true, true, 168);
		addSprite(52506, 56);
		tab.totalChildren(9);
		...
		for (int i = 0; i < 30; i++) {
			addHoverText(52531 + i, "", "Open shop", daniel, 0, 0xE3A724, true, true, 168, 0xFFFFFF);
	 ...
	 * @param Mayhem
	 */
	private static void shop(TextDrawingArea[] Mayhem) {
		...
		addText(19679, "", 0xff981f, false, true, 52, Mayhem, 1);
		addText(19680, "", 0xbf751d, false, true, 52, Mayhem, 1);
		addButton(19681, 2, "Interfaces/Shop/SHOP", 0, 0, "", 1);
		addSprite(19687, 1, "Interfaces/Shop/ITEMBG");
		for (int i = 0; i < 36; i++) {
			addText(28000 + i, "-1,0", 0xffff00, false, false, 52, Mayhem, 0);
		}
		...
		addText(19684, "Main Stock", 0xbf751d, false, true, 52, Mayhem, 1);
		addText(19685, "Store Info", 0xff981f, false, true, 52, Mayhem, 1);
	...
	public static void trainingTeleport(TextDrawingArea[] Mayhem) {
		...
		addText(61023, "@or2@Teleportation Menu (@red@Training/Slayer@or2@)", 0xff9933, true, true, -1, Mayhem, 2);
		addText(61024, "@gre@Training/Slayer", 0xff9933, true, true, -1, Mayhem, 1);
		addText(61025, "Skilling", 0xff9933, true, true, -1, Mayhem, 1);
		addText(61026, "Player Vs Player", 0xff9933, true, true, -1, Mayhem, 1);
		addText(61027, "Bosses", 0xff9933, true, true, -1, Mayhem, 1);
		addText(61028, "Minigames", 0xff9933, true, true, -1, Mayhem, 1);
		addText(61029, "Cities & Shops", 0xff9933, true, true, -1, Mayhem, 1);
		addText(61030, "Teleport Information:", 0xff9933, true, true, -1, Mayhem, 2);
		addText(61031, "Selected: @red@None", 0xff9933, false, true, -1, Mayhem, 0);
		addText(61032, "Cost: @red@Free", 0xff9933, false, true, -1, Mayhem, 0);
		addText(61033, "Requirement: @red@None", 0xff9933, false, true, -1, Mayhem, 0);
		addText(61034, "Other: @red@None", 0xff9933, false, true, -1, Mayhem, 0);
		...
		int y = 0;
		for (int i = 0; i < 40; i++) {
			addHoverText(61051 + i, "", "Select", Mayhem, 1, 0xff9633, true, true, 300);
	...
	public static void skillingTeleport(TextDrawingArea[] Mayhem) {
		...
		addText(62023, "@or2@Teleportation Menu (@red@Skilling@or2@)", 0xff9933, true, true, -1, Mayhem, 2);
		addText(62024, "Training/Slayer", 0xff9933, true, true, -1, Mayhem, 1);
		addText(62025, "@gre@Skilling", 0xff9933, true, true, -1, Mayhem, 1);
		addText(62026, "Player Vs Player", 0xff9933, true, true, -1, Mayhem, 1);
		addText(62027, "Bosses", 0xff9933, true, true, -1, Mayhem, 1);
		addText(62028, "Minigames", 0xff9933, true, true, -1, Mayhem, 1);
		addText(62029, "Cities & Shops", 0xff9933, true, true, -1, Mayhem, 1);
		addText(62030, "Teleport Information:", 0xff9933, true, true, -1, Mayhem, 2);
		addText(62031, "Selected: @red@None", 0xff9933, false, true, -1, Mayhem, 0);
		addText(62032, "Cost: @red@Free", 0xff9933, false, true, -1, Mayhem, 0);
		addText(62033, "Requirement: @red@None", 0xff9933, false, true, -1, Mayhem, 0);
		addText(62034, "Other: @red@None", 0xff9933, false, true, -1, Mayhem, 0);
		...
		int y = 0;
		for (int i = 0; i < 40; i++) {
			addHoverText(62051 + i, "", "Select", Mayhem, 1, 0xff9633, true, true, 300);
	...
	public static void pvpTeleport(TextDrawingArea[] Mayhem) {
		...
		addText(63023, "@or2@Teleportation Menu (@red@Player Vs Player@or2@)", 0xff9933, true, true, -1, Mayhem, 2);
		addText(63024, "Training/Slayer", 0xff9933, true, true, -1, Mayhem, 1);
		addText(63025, "Skilling", 0xff9933, true, true, -1, Mayhem, 1);
		addText(63026, "@gre@Player Vs Player", 0xff9933, true, true, -1, Mayhem, 1);
		addText(63027, "Bosses", 0xff9933, true, true, -1, Mayhem, 1);
		addText(63028, "Minigames", 0xff9933, true, true, -1, Mayhem, 1);
		addText(63029, "Cities & Shops", 0xff9933, true, true, -1, Mayhem, 1);
		addText(63030, "Teleport Information:", 0xff9933, true, true, -1, Mayhem, 2);
		addText(63031, "Selected: @red@None", 0xff9933, false, true, -1, Mayhem, 0);
		addText(63032, "Cost: @red@Free", 0xff9933, false, true, -1, Mayhem, 0);
		addText(63033, "Requirement: @red@None", 0xff9933, false, true, -1, Mayhem, 0);
		addText(63034, "Other: @red@None", 0xff9933, false, true, -1, Mayhem, 0);
		...
		int y = 0;
		for (int i = 0; i < 40; i++) {
			addHoverText(63051 + i, "", "Select", Mayhem, 1, 0xff9633, true, true, 300);
	...
	public static void bossTeleport(TextDrawingArea[] Mayhem) {
		...
		addText(64023, "@or2@Teleportation Menu (@red@Bosses@or2@)", 0xff9933, true, true, -1, Mayhem, 2);
		addText(64024, "Training/Slayer", 0xff9933, true, true, -1, Mayhem, 1);
		addText(64025, "Skilling", 0xff9933, true, true, -1, Mayhem, 1);
		addText(64026, "Player Vs Player", 0xff9933, true, true, -1, Mayhem, 1);
		addText(64027, "@gre@Bosses", 0xff9933, true, true, -1, Mayhem, 1);
		addText(64028, "Minigames", 0xff9933, true, true, -1, Mayhem, 1);
		addText(64029, "Cities & Shops", 0xff9933, true, true, -1, Mayhem, 1);
		addText(64030, "Teleport Information:", 0xff9933, true, true, -1, Mayhem, 2);
		addText(64031, "Selected: @red@None", 0xff9933, false, true, -1, Mayhem, 0);
		addText(64039, "Level: @red@0", 0xff9933, false, true, -1, Mayhem, 0);
		addText(64032, "Cost: @red@Free", 0xff9933, false, true, -1, Mayhem, 0);
		addText(64033, "Requirement: @red@None", 0xff9933, false, true, -1, Mayhem, 0);
		addText(64034, "Other: @red@None", 0xff9933, false, true, -1, Mayhem, 0);
		...
		int y = 0;
		for (int i = 0; i < 40; i++) {
			addHoverText(64051 + i, "", "Select", Mayhem, 1, 0xff9633, true, true, 300);
	...
	public static void minigameTeleport(TextDrawingArea[] Mayhem) {
		...
		addText(65023, "@or2@Teleportation Menu (@red@Minigames@or2@)", 0xff9933, true, true, -1, Mayhem, 2);
		addText(65024, "Training/Slayer", 0xff9933, true, true, -1, Mayhem, 1);
		addText(65025, "Skilling", 0xff9933, true, true, -1, Mayhem, 1);
		addText(65026, "Player Vs Player", 0xff9933, true, true, -1, Mayhem, 1);
		addText(65027, "Bosses", 0xff9933, true, true, -1, Mayhem, 1);
		addText(65028, "@gre@Minigames", 0xff9933, true, true, -1, Mayhem, 1);
		addText(65029, "Cities & Shops", 0xff9933, true, true, -1, Mayhem, 1);
		addText(65030, "Teleport Information:", 0xff9933, true, true, -1, Mayhem, 2);
		addText(65031, "Selected: @red@None", 0xff9933, false, true, -1, Mayhem, 0);
		addText(65032, "Cost: @red@Free", 0xff9933, false, true, -1, Mayhem, 0);
		addText(65033, "Requirement: @red@None", 0xff9933, false, true, -1, Mayhem, 0);
		addText(65034, "Other: @red@None", 0xff9933, false, true, -1, Mayhem, 0);
		...
		int y = 0;
		for (int i = 0; i < 40; i++) {
			addHoverText(65051 + i, "", "Select", Mayhem, 1, 0xff9633, true, true, 300);
	...
	public static void otherTeleport(TextDrawingArea[] Mayhem) {
		...
		addText(61523, "@or2@Teleportation Menu (@red@Cities & Shops@or2@)", 0xff9933, true, true, -1, Mayhem, 2);
		addText(61524, "Training/Slayer", 0xff9933, true, true, -1, Mayhem, 1);
		addText(61525, "Skilling", 0xff9933, true, true, -1, Mayhem, 1);
		addText(61526, "Player Vs Player", 0xff9933, true, true, -1, Mayhem, 1);
		addText(61527, "Bosses", 0xff9933, true, true, -1, Mayhem, 1);
		addText(61528, "Minigames", 0xff9933, true, true, -1, Mayhem, 1);
		addText(61529, "@gre@Cities & Shops", 0xff9933, true, true, -1, Mayhem, 1);
		addText(61530, "Teleport Information:", 0xff9933, true, true, -1, Mayhem, 2);
		addText(61531, "Selected: @red@None", 0xff9933, false, true, -1, Mayhem, 0);
		addText(61532, "Cost: @red@Free", 0xff9933, false, true, -1, Mayhem, 0);
		addText(61533, "Requirement: @red@None", 0xff9933, false, true, -1, Mayhem, 0);
		addText(61534, "Other: @red@None", 0xff9933, false, true, -1, Mayhem, 0);
		...
		int y = 0;
		for (int i = 0; i < 40; i++) {
			addHoverText(61551 + i, "", "Select", Mayhem, 1, 0xff9633, true, true, 300);
	...
	public static void optionTab(TextDrawingArea[] Mayhem) {
		...
		addText(36016, "", Mayhem, 1, 0xff9933, true, true);
		addText(36003, "Options Tab", Mayhem, 1, 0xff9933, true, true);
		addText(36013, "Fixed", Mayhem, 0, 0xff9933, true, true);
		addText(36014, "Resizable", Mayhem, 0, 0xff9933, true, true);
		addText(36015, "Fullscreen", Mayhem, 0, 0xff9933, true, true);
		addHoverText(36025, "Screen Options", "Screen Options", Mayhem, 0, 0xff981f, true, true, 60);
	 ...
	 * @param Mayhem
	 */
	public static void quickPrayers(TextDrawingArea[] Mayhem) {
		int frame = 0;
	 ...
	 * @param Mayhem
	 */
	public static void quickCurses(TextDrawingArea[] Mayhem) {
		RSInterface tab = addTabInterface(17234);
		addTransparentSprite(17229, 444);
		addSprite(17201, 445);
		addText(17230, "Select your quick prayers:", Mayhem, 0, 0xFF981F, false, true);
	 ...
	 * @param Mayhem
	 */
	public static void equipmentScreen(TextDrawingArea[] Mayhem) {
		...
		addText(15111, "", Mayhem, 2, 0xe4a146, false, true);
		addText(15112, "Attack bonus", Mayhem, 2, 0xFF981F, false, true);
		addText(15113, "Defence bonus", Mayhem, 2, 0xFF981F, false, true);
		addText(15114, "Other bonuses", Mayhem, 2, 0xFF981F, false, true);
		addText(15115, "Equipment Bonuses", Mayhem, 2, 0xFF981F, false, true);
		addText(15116, "", Mayhem, 0, 0xFF981F, true, true);
		addText(15117, "", Mayhem, 0, 0xFF981F, true, true);
		addText(15118, "", Mayhem, 0, 0xFF981F, true, true);
		for (int i = 1675; i <= 1684; i++) {
			textSize(i, Mayhem, 1);
		}
		textSize(1686, Mayhem, 1);
		textSize(1687, Mayhem, 1);
		addChar(15125, 560);
	 ...
	 * @param Mayhem
	 */
	public static void equipmentTab(TextDrawingArea[] Mayhem) {
	 ...
	 * @param Mayhem
	 */
	public static void clanChatTab(TextDrawingArea[] Mayhem) {
		...
		addText(18135, "Join/Leave", Mayhem, 0, 0xff9b00, true, true);
		addText(18136, "Clan Setup", Mayhem, 0, 0xff9b00, true, true);
		addSprite(18137, 42);
		addText(18138, "Clan Chat", Mayhem, 2, 0xff9b00, true, true);
		addText(18139, "Talking in: Not in chat", Mayhem, 0, 0xff9b00, false, true);
		addText(18140, "Owner: None", Mayhem, 0, 0xff9b00, false, true);
		
		addText(18252, "(0/100)", Mayhem, 0, 0xff9b00, false, true);
		tab.totalChildren(14);
		...
		for (int i = 18144; i <= 18244; i++) {
			addText(i, "", Mayhem, 0, 0xffffff, false, true);
		}
	 ...
	 * @param Mayhem
	 */
	public static void clanChatSetup(TextDrawingArea[] Mayhem) {
		...
		/* Clan Setup title */
		addText(43703, "Clan Setup", Mayhem, 2, 0xFF981F, true, true);
		...
		for (int index = 0, id = 43704, y = 50; index < titles.length; index++, id += 3, y += 40) {
			...
			addText(id + 1, titles[index], Mayhem, 0, 0xFF981F, true, true);
			addText(id + 2, defaults[index], Mayhem, 1, 0xFFFFFF, true, true);
		...
		int id = 43720;
		int y = 74;
		addText(id, "Ranked Members", Mayhem, 2, 0xFF981F, false, true);
		rsi.child(count++, id++, 202, y);
		addText(id, "Banned Members", Mayhem, 2, 0xFF981F, false, true);
		...
		for (int index = id; index < id + lines; index++) {
			addText(index, "", Mayhem, 1, 0xffffff, false, true);
		...
		/* Table info text */
		y = 47;
		addText(id, "You can manage both ranked and banned members here.", Mayhem, 0, 0xFF981F, true, true);
		rsi.child(count++, id++, 337, y);
		addText(id, "Right click on a name to edit the member.", Mayhem, 0, 0xFF981F, true, true);
	 ...
	 * @param Mayhem
	 */
	/*public static void questTab(TextDrawingArea[] Mayhem) {
		RSInterface tab = addTabInterface(29400);
		addSprite(29401, 82);
		addSprite(29402, 38);
		addText(29403, "Valius", Mayhem, 2, 0xF7AA25, true, true);
		addText(29407, "www.Valius.Org", Mayhem, 2, 0xF7AA25, true, true);
		...
		addHoverText(29413, "", "", Mayhem, 0, 0x47B320, false, true, 50);
		tab.scrollMax = 0;
		tab.totalChildren(11);
		...
		scrollInterface.totalChildren(amountOfLines);
		for (int i = 0; i < amountOfLines; i++) {
			addHoverText(29501 + i, "", "", Mayhem, 0, 0x47B320, false, true, 168);
